...
export async function getTokenBalance(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (!accountResource) {
        throw new Error(
          `Could not find APT coin store for address: ${address}`,
        );
      }
  ...
  throw new Error(
    `Account ${address} not found on the Aptos network after ${maxRetries} attempts. The account may not exist or may not be funded yet.`,
  );
}
